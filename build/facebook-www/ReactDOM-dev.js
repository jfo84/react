...
var enableSelectiveHydration = _require.enableSelectiveHydration; // In www, we have experimental support for gathering data
...
var getInstanceFromNode$2 = null;
...
function setComponentTree(
  ...
  getInstanceFromNode$2 = getInstanceFromNodeImpl;
  getNodeFromInstance$1 = getNodeFromInstanceImpl;

  {
    !(getNodeFromInstance$1 && getInstanceFromNode$2)
      ? warningWithoutStack(
          false,
...
function restoreStateOfTarget(target) {
  ...
  var internalInstance = getInstanceFromNode$2(target);
  ...
  var props = getFiberCurrentPropsFromNode$1(internalInstance.stateNode);
...
function validateResponderContext() {
  if (!(currentInstance !== null)) {
    {
      throw ReactError(
        Error("An event responder context was used outside of an event cycle.")
      );
    }
  }
}
    ...
    if (children != null) {
      ...
      if (Array.isArray(children)) {
        if (!(children.length <= 1)) {
          {
            throw ReactError(
              Error("<textarea> can only have at most one child.")
            );
          }
        }
...
var runWithPriority$2 = Scheduler.unstable_runWithPriority;
...
function dispatchEventForPluginEventSystem(
  ...
  try {
    ...
    // `preventDefault`.
    batchedEventUpdates$1(handleTopLevel, bookKeeping);
  } finally {
...
function attemptSynchronousHydration$1(fiber) {
  switch (fiber.tag) {
    ...
    case SuspenseComponent:
      ...
      var retryExpTime = computeInteractiveExpiration(requestCurrentTime());
...
function attemptUserBlockingHydration$1(fiber) {
  ...
  var expTime = computeInteractiveExpiration(requestCurrentTime());
...
function attemptContinuousHydration$1(fiber) {
  ...
  var expTime = computeContinuousHydrationExpiration(requestCurrentTime());
  scheduleWork(fiber, expTime);
  markRetryTimeIfNotHydrated(fiber, expTime);
}
...
function manualDispatchChangeEvent(nativeEvent) {
  ...
  batchedUpdates$1(runEventInBatch, event);
}
...
ReactRoot.prototype.render = ReactSyncRoot.prototype.render = function(
  ...
  {
    warnOnInvalidCallback$1(callback, "render");
  }
