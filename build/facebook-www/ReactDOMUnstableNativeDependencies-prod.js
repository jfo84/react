...
function executeDirectDispatch(event) {
  ...
  if (Array.isArray(dispatchListener)) throw ReactErrorProd(Error(103));
...
function getTouchIdentifier(_ref) {
  ...
  if (null == _ref) throw ReactErrorProd(Error(138));
  return _ref;
}
...
function accumulate(current, next) {
  if (null == next) throw ReactErrorProd(Error(334));
  return null == current
    ? next
